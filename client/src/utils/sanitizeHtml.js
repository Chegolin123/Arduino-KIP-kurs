...
function isSafeUrl(url, tagName) {
  ...
  if (/^https?:\/\//i.test(value) || /^mailto:/i.test(value) || /^tel:/i.test(value)) return true;
  if (tagName === 'img' && /^data:image\//i.test(value)) return true;
  return false;
}
...
function sanitizeAttributes(tagName, attrsText) {
  ...
  while ((match = attrRegex.exec(attrsText)) !== null) {
    ...
    if (name === 'target' && value !== '_blank' && value !== '_self' && value !== '_parent' && value !== '_top') continue;
